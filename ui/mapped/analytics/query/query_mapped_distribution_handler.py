...
def _render_navigation_header(st):
    ...
    st.header("🔍 Query MAPPED Dataset")
    ...
    if st.button("🏠 Torna alla Distribution", key="back_to_distribution"):
        ...
        st.session_state.current_stage = "mapped_distribution_main"
        st.rerun()
...
def _render_query_interface(st, data_path: str, file_extension: str):
    ...
    supported_extensions = ['.json', '.jsonl', '.csv', '.gz', '.parquet', '.jsonl.gz']  # Aggiunto punto
    files = [f for f in Path(data_path).iterdir() if f.suffix.lower() in [ext.lower() for ext in supported_extensions] and f.is_file()]

    if not files:
        st.warning("⚠️ Nessun file supportato trovato")
        return
    ...
    # --- NUOVA SEZIONE: Nome cartella risultato ---
    ...
    st.session_state.result_folder_name = st.text_input(
        ...
        help=f"Inserisci il nome della cartella dove verranno salvati i risultati. Sarà creata in: {data_path}"
    )
    
    # Area query SQL
    st.markdown("**Scrivi la tua query SQL:**")
    example_query = f"SELECT * FROM '{data_path}/*{file_extension}' AS t"
    ...
    with col_exec1:
        limit_results = st.number_input(
            ...
            min_value=1,
            value=st.session_state.get("limit_results_input", 100),
...
def _execute_sql_query(st):
    """Esegue la query SQL e gestisce i risultati con gestione dinamica di _subpath."""
    ...
    try:
        ...
        with duckdb.connect() as conn:
            ...
            try:
                probe_df = conn.execute(f"SELECT * FROM ({query_with_limit}) AS t LIMIT 0").fetchdf()
                has_subpath = "_subpath" in probe_df.columns
            except Exception as probe_err:
                # Se fallisce il probe (es. errore sintassi nella query base), 
                # solleviamo l'eccezione per catturarla nel blocco principale
                raise probe_err
...
"""
                    st.info(f"✨ Trasformazione applicata su `_subpath` esistente (folder: '{folder_name}')")
                else:
                    # CASO B: _subpath NON esiste. Lo creiamo come colonna costante
                    final_query = f"""
SELECT 
    t.*,
...
"""
                ...
                st.info("✨ Esecuzione query originale (nessuna modifica a _subpath)")
            ...
            # Debug log su console
            print("--- EXECUTION LOG ---")
            print(f"Original exists _subpath: {has_subpath}")
            ...
            st.success(f"✅ Query completata: {len(result)} righe caricate.")
        ...
        # Mostra la query che ha fallito per debug
        if 'final_query' in locals():
            st.info(f"💡 **Query generata che ha causato l'errore:**\n```sql\n{final_query}\n```")
        
        # Suggerimenti contestuali
        err_msg = str(e).lower()
        if "no such file" in err_msg or "could not open" in err_msg:
            st.info("💡 **Suggerimento**: Il file specificato nel FROM non è stato trovato o è inaccessibile.")
        elif "binder error" in err_msg:
            st.info("💡 **Suggerimento**: Problema di riferimento alle colonne. Verifica che i nomi dei campi siano corretti.")
...
def _handle_save_confirmation(st, result, destination_path: Path, materialize_dataset: bool, repos: Dict):
    ...
    if success and not materialize_dataset:
        ...
        st.session_state.save_success_msg = f"✅ Risultati salvati e distribuzione creata con successo in `{destination_path}`"
        st.session_state.save_state = 1  # Stato di successo
    else:
    ...
    try:
        ...
        if old_dataset.uri == current_dist.uri:
            ...
            new_dataset = Dataset(
                ...
                derived_dataset=current_dist.dataset_id,  
        ...
        query_original = st.session_state.get('original_query', '')
        user_limit = st.session_state.get("limit_results_input", 100)
...
def show_mapped_query_dataset(st):
    ...
    file_extension = _get_file_extension(distribution_path)
    if not file_extension:
        st.warning("⚠️ Nessun file trovato nella cartella del dataset.")
        return
    
        
    _render_main_interface(st, distribution_path, file_extension, repos)

## INTERFACE RENDERING AND HANDLING ##

def _render_main_interface(st, data_path: str, 
