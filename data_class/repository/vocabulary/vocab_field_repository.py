# repository/vocab_field_repository.py
import logging
...
class VocabFieldRepository:
    def __init__(self, db_manager: PostgresDBManager):
        self.db = db_manager
        self.table = POSTGRES_DB_SCHEMA + "." + "vocab_field"

    def _prepare_params(self, entity: VocabField, include_id: bool = False) -> Dict[str, Any]:
        params = {
            "code": entity.code,
    ...
    def insert(self, entity: VocabField) -> Optional[VocabField]:
        ...
        with self.db as db:
            ...
            return VocabField(**row) if row else None

    def get_by_id(self, id: str) -> Optional[VocabField]:
        query = f"SELECT * FROM {self.table} WHERE id = %s"
        with self.db as db:
            row = db.execute_and_return(query, (id,))
            return VocabField(**row) if row else None

    def get_by_code(self, code: str) -> Optional[VocabField]:
        query = f"SELECT * FROM {self.table} WHERE code = %s"
        with self.db as db:
            row = db.execute_and_return(query, (code,))
            return VocabField(**row) if row else None

    def get_all(self) -> List[VocabField]:
        query = f"SELECT * FROM {self.table} ORDER BY code ASC"
        with self.db as db:
            rows = db.query(query)
            return [VocabField(**row) for row in rows]

    def update(self, entity: VocabField) -> int:
        if not entity.id:
            raise ValueError("ID mancante per l'aggiornamento della field")
