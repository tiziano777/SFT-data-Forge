# repository/vocab_content_repository.py
import logging
...
class VocabContentRepository:
    def __init__(self, db_manager: PostgresDBManager):
        self.db = db_manager
        self.table = POSTGRES_DB_SCHEMA + "." + "vocab_content"

    def _prepare_params(self, entity: VocabContent, include_id: bool = False) -> Dict[str, Any]:
        params = {
            "code": entity.code,
    ...
    def insert(self, entity: VocabContent) -> Optional[VocabContent]:
        ...
        with self.db as db:
            ...
            return VocabContent(**row) if row else None

    def get_by_id(self, id: str) -> Optional[VocabContent]:
        query = f"SELECT * FROM {self.table} WHERE id = %s"
        with self.db as db:
            row = db.execute_and_return(query, (id,))
            return VocabContent(**row) if row else None

    def get_by_code(self, code: str) -> Optional[VocabContent]:
        query = f"SELECT * FROM {self.table} WHERE code = %s"
        with self.db as db:
            row = db.execute_and_return(query, (code,))
            return VocabContent(**row) if row else None

    def get_all(self) -> List[VocabContent]:
        query = f"SELECT * FROM {self.table} ORDER BY code ASC"
        with self.db as db:
            rows = db.query(query)
            return [VocabContent(**row) for row in rows]

    def update(self, entity: VocabContent) -> int:
        if not entity.id:
            raise ValueError("ID mancante per l'aggiornamento della content")
