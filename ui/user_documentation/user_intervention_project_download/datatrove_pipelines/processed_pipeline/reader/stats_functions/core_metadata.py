...
def _detect_lang(data_dict: dict, available_langs: list) -> str:
    ...
    # Se il testo non c'è, fallback sulla prima disponibile
...
def default_adapter_aux(data: dict, path: str, id_in_file: int | str, 
                       default_metadata: Dict[str, Any] = None) -> Dict[str, Any]:
    try:
        # 1. Pulizia e Testo Completo (per ID e Document Text)
        cleaned_data = _clean_data_dict(data)
        full_text_content = _extract_text_from_data(cleaned_data)
        document_id = _compute_id_hash(full_text_content)

        default_meta = default_metadata or {}
        available_langs = default_meta.get("_available_languages", [])

        # 2. Language Detection con la nuova euristica (passiamo il dict, non il testo)
        ...
        logger.error(f"❌ Errore adapter: {e}")
        raise e
