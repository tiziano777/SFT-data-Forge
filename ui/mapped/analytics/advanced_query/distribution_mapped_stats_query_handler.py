...
def _get_distribution_paths(distribution: Distribution) -> tuple:
    ...
    chat_stats = data_path.replace(BINDED_MAPPED_DATA_DIR, BINDED_STATS_DATA_DIR) + CHAT_TEMPLATE_STATS_EXTENSION
    return to_internal_path(data_path), to_internal_path(stats_path), to_internal_path(chat_stats)

### STATS EXTRACTION AND CALCULATION FUNCTIONS ###

def _extract_stats_columns(stats_path: str, table_type: str = "stats") -> List[str]:
    """Estrae dinamicamente le colonne dalle statistiche leggendo il primo file parquet.
    
    Args:
        stats_path: Percorso alla directory delle statistiche
        table_type: Tipo di tabella ('stats' o 'chat_stats')
    """
    try:
        ...
        # Aggiungi prefisso per identificare la tabella di origine
        prefixed_columns = []
        for col in columns:
            if table_type == "stats":
                prefixed_columns.append(f"s.{col}")
            elif table_type == "chat_stats":
                prefixed_columns.append(f"c.{col}")
            else:
                prefixed_columns.append(col)
        
        return prefixed_columns
    except Exception as e:
        logger.error(f"Error extracting stats columns from {stats_path}: {e}")
        return []
...
def _calculate_column_stats(stats_path: str, column: str) -> Dict[str, Any]:
    ...
    Args:
        stats_path: Percorso alla directory delle statistiche
        column: Nome della colonna (può includere prefisso tipo 's.' o 'c.')
        table_type: Tipo di tabella ('stats' o 'chat_stats')
    """
    try:
        # Rimuovi prefisso per la query
        clean_column = column.split('.')[-1] if '.' in column else column
        
        conn = duckdb.connect(':memory:')
        query = f"""
            SELECT 
                MIN({clean_column}) as min_val,
                MAX({clean_column}) as max_val,
                AVG({clean_column}) as mean_val,
                MEDIAN({clean_column}) as median_val,
                PERCENTILE_CONT(0.05) WITHIN GROUP (ORDER BY {clean_column}) as p05,
                PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY {clean_column}) as p95,
                STDDEV({clean_column}) as std_dev,
                COUNT({clean_column}) as count_val
        ...
        logger.error(f"Error calculating stats for {column} from {stats_path}: {e}")
        return {}
...
def _render_stats_visualization(st_app, stats_path: str, chat_stats: str, stats_columns: List[str]):
    ...
    with st_app.expander("ℹ️ Info Template Chat Stats", expanded=False):
        st_app.markdown("""
                ### 📊 Metriche del Template
                
                | Metrica | Descrizione |
                | :--- | :--- |
                | `_message_count` | Numero totale di messaggi (turns) |
                | `_has_system` | Presenza di system prompt |
                | `_has_context` | Presenza di context aggiuntivo |
                | `_context_count` | Numero di messaggi di context |
                | `_has_think` | Presenza di ragionamento (thought) |
                | `_has_functioncall` | Presenza di function calls |
                | `_user_ends_with_question` | L'ultimo messaggio utente termina con '?' |
                | **Analisi Contenuto** | |
                | `_curly_brackets_count` | Parentesi graffe (indicatori JSON) nel contenuto |
                | `_think_steps_count` | Complessità ragionamento (linee di think) |
                | `_think_message_count` | Numero messaggi con blocco think |
                | `_functioncall_message_count` | Numero di chiamate a funzioni rilevate |
                | `_user_code_count` | Blocchi di codice nei messaggi utente |
                | `_html_tag_count` | Numero di tag HTML rilevati |
                | `_xml_tag_count` | Numero di tag XML rilevati |
                | **AI Features** | |
                | `_assistant_has_json` | Rilevato formato JSON nella risposta |
                | `_assistant_has_code` | Rilevato codice nella risposta |
                | `_assistant_has_markdown` | Rilevato markdown nella risposta |
                | `_assistant_has_table` | Rilevata tabella nella risposta |
                | `_assistant_has_math` | Rilevate formule matematiche (LaTeX) |
                | **Volumi (Char/Words)** | |
                | `_user_char_count` | Caratteri totali utente |
                | `_assistant_char_count` | Caratteri totali assistant |
                | `_system_char_count` | Caratteri totali sistema |
                | `_think_char_count` | Caratteri totali nel blocco think |
                | `_user_word_count` | Parole totali utente |
                | `_assistant_word_count` | Parole totali assistant |
                | `_system_word_count` | Parole totali sistema |
                | **Tokenizzazione** | |
                | `_user_token_count` | Token totali utente |
                | `_assistant_token_count` | Token totali assistant |
                | `_system_token_count` | Token totali sistema |
                | `_context_token_count` | Token totali nel contesto |
                | **Medie e Ratio** | |
                | `_user_word_mean` | Media parole per messaggio utente |
                | `_assistant_word_mean` | Media parole per messaggio assistant |
                | `_user_avg_length` | Lunghezza media caratteri utente |
                | `_assistant_avg_length` | Lunghezza media caratteri assistant |
                | `_user_assistant_ratio` | Ratio caratteri utente/assistant |
                | `_think_to_content_ratio` | Ratio tra think e risposta finale |
                | `_last_interaction_ratio` | Peso dell'ultima interazione |
                """)
    
    # Seleziona tabella di origine per le statistiche
    table_source = st_app.selectbox(
        "Seleziona tabella di statistiche:",
        options=["Stats (s)", "Chat Stats (c)"],
        format_func=lambda x: "Statistiche Generali" if x == "Stats (s)" else "Statistiche Chat Template"
    )
    
    # Filtra colonne in base alla tabella selezionata
    if table_source == "Stats (s)":
        filtered_columns = [col for col in stats_columns if col.startswith('s.')]
        current_stats_path = stats_path
    else:
        filtered_columns = [col for col in stats_columns if col.startswith('c.')]
        current_stats_path = chat_stats
    
    if not filtered_columns:
        st_app.warning(f"⚠️ Nessuna colonna trovata nella tabella {table_source}")
        return
    ...
    if selected_column:
        _render_column_visualization(st_app, current_stats_path, selected_column)
    ...
    with st_app.spinner("Caricamento dati e calcolo statistiche..."):
        ...
        with col2:
            ...
            metrics_data = [
                ...
                ("📍 5th %ile", f"{float(stats['p05']):.2f}"),
                ("📍 95th %ile", f"{float(stats['p95']):.2f}"),
                ("📏 Std Dev", f"{float(stats['std']):.2f}"),
                ("🎯 Median", f"{float(stats['median']):.2f}"),
            ]
            with subcol1:
...
def _build_filter_condition(column: str, filter_type: str, value: Any) -> str:
    ...
    # Assicurati che la colonna abbia l'alias della tabella
    if '.' not in column:
        # Se non ha prefisso, prova a determinarlo
        # In una implementazione reale, potresti voler verificare in quale tabella esiste la colonna
        column = f"s.{column}"
...
def _render_query_builder(st_app, data_path: str, stats_path: str, chat_stats: str, stats_columns: List[str], file_extension: str):
    ...
    # Raggruppa colonne per tabella
    stats_table_columns = [col for col in stats_columns if col.startswith('s.')]
    chat_stats_columns = [col for col in stats_columns if col.startswith('c.')]
    
    # Seleziona tabella di origine
    table_choice = st_app.selectbox(
        "Tabella sorgente:",
        options=["Stats (s)", "Chat Stats (c)", "Entrambe"],
        format_func=lambda x: {
            "Stats (s)": "Statistiche Generali",
            "Chat Stats (c)": "Statistiche Chat Template",
            "Entrambe": "Qualsiasi Tabella"
        }[x]
    )
    
    # Filtra colonne in base alla scelta
    if table_choice == "Stats (s)":
        available_columns = stats_table_columns
    elif table_choice == "Chat Stats (c)":
        available_columns = chat_stats_columns
    else:
        available_columns = stats_columns
    ...
    if st_app.session_state.active_filters:
        ...
        for idx, f in enumerate(st_app.session_state.active_filters):
            ...
            with col1:
                table_display = {
                    "Stats (s)": "📊 Stats",
                    "Chat Stats (c)": "💬 Chat Stats",
                    "Entrambe": "🔗 Qualsiasi"
                }[f['table']]
                
                if f['type'] == 'range':
                    st_app.info(f"{table_display} | **{f['column']}** {f['type']}: [{f['value'][0]}, {f['value'][1]}]")
                elif f['type'] in ['in', 'not in']:
                    st_app.info(f"{table_display} | **{f['column']}** {f['type']}: {f['value']}")
                else:
                    st_app.info(f"{table_display} | **{f['column']}** {f['type']} {f['value']}")
            with col2:
    ...
    custom_query = st_app.text_area(
        ...
        height=100,
        placeholder="Es: d.text_length > 1000 AND s.word_count < 500 AND c.template_score > 0.8",
...
def _execute_and_display_query(st_app, data_path: str, stats_path: str, chat_stats: str,
    ...
    try:
        ...
        query = _build_visual_query(data_path, file_extension, stats_path, chat_stats, filters, custom_query, folder_name)
        ...
        with st_app.spinner("Esecuzione query..."):
            ...
            logger.info(f"Executing query on {file_extension} files with triple join...")
...
def _build_visual_query(data_path: str, file_extension: str, stats_path: str, chat_stats: str,
    ...
    chat_stats_pattern = f"'{chat_stats}/*.parquet'"
...
def show_mapped_distribution_chat_stats_query_handler(st_app):
    ...
    try:
        ...
        data_path, stats_path, chat_stats_path = _get_distribution_paths(distribution)
        ...
        # Estrai colonne da entrambe le tabelle con prefissi
        stats_columns = _extract_stats_columns(stats_path, "stats")
        chat_stats_columns = _extract_stats_columns(chat_stats_path, "chat_stats")
        
        # Combina le colonne
        all_columns = stats_columns + chat_stats_columns
        
        if not all_columns:
            st_app.warning("⚠️ Nessuna colonna di statistiche trovata in nessuna tabella")
            return
        
        # Info distribution
        st_app.info(f"📦 Distribution: **{distribution.name}** | 📊 {len(stats_columns)} statistiche generali | 💬 {len(chat_stats_columns)} statistiche chat template")
        ...
        if st_app.session_state.query_mode == "visualize_stats":
            _render_stats_visualization(st_app, stats_path, chat_stats_path, all_columns)
        elif st_app.session_state.query_mode == "build_query":
            _render_query_builder(st_app, data_path, stats_path, chat_stats_path, all_columns, file_extension)
