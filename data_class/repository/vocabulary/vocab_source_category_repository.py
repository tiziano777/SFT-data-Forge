# repository/vocab_source_category_repository.py
import logging
...
class VocabSourceCategoryRepository:
    def __init__(self, db_manager: PostgresDBManager):
        self.db = db_manager
        self.table = POSTGRES_DB_SCHEMA + "." + "vocab_source_category"

    def _prepare_params(self, entity: VocabSourceCategory, include_id: bool = False) -> Dict[str, Any]:
        params = {
            "code": entity.code,
    ...
    def insert(self, entity: VocabSourceCategory) -> Optional[VocabSourceCategory]:
        ...
        with self.db as db:
            ...
            return VocabSourceCategory(**row) if row else None

    def get_by_id(self, id: str) -> Optional[VocabSourceCategory]:
        query = f"SELECT * FROM {self.table} WHERE id = %s"
        with self.db as db:
            row = db.execute_and_return(query, (id,))
            return VocabSourceCategory(**row) if row else None

    def get_by_code(self, code: str) -> Optional[VocabSourceCategory]:
        query = f"SELECT * FROM {self.table} WHERE code = %s"
        with self.db as db:
            row = db.execute_and_return(query, (code,))
            return VocabSourceCategory(**row) if row else None

    def get_all(self) -> List[VocabSourceCategory]:
        query = f"SELECT * FROM {self.table} ORDER BY code ASC"
        with self.db as db:
            rows = db.query(query)
            return [VocabSourceCategory(**row) for row in rows]

    def update(self, entity: VocabSourceCategory) -> int:
        if not entity.id:
            raise ValueError("ID mancante per l'aggiornamento della source category")
