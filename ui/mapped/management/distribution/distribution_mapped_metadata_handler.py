...
def _render_metadata_form(st, distribution: Distribution, vocab_options: Dict):
    ...
    with st.form("metadata_form", clear_on_submit=False):
        ...
        tokenized_uri = st.text_input("URI Tokenized", value=distribution.tokenized_uri or "", key="tokenized_uri_input")
        version = st.text_input("Versione *", value=distribution.version, key="version_input")

        # Legge le opzioni del vocabolario per distribution split
        ...
        with col2:
            ...
            #gen = st.form_submit_button("⚙️ Genera automaticamente", key="generate_button", disabled=True)
        with col3:
...
def show_metadata_editor(st):
    ...
    '''
        ...
        st.session_state.current_stage = "mapped_metadata_generation"
        st.rerun()
    '''
