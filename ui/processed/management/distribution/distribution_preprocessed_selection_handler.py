# ui/processed/management/distribution/distribution_preprocessed_selection_handler.py
import os
import logging
...
def _build_dataset_labels(datasets: List[Dataset]) -> List[str]:
    ...
    return [
        f"{dataset.name} - {dataset.version} - {dataset.uri.replace(BASE_PREFIX + PROCESSED_DATA_DIR, '')}" 
...
def _build_unmaterialized_distribution_labels(distributions: List[Distribution]) -> List[str]:
    ...
    for dist in distributions:
        ...
        if uri.startswith(BASE_PREFIX):
            ...
            if clean_path.startswith(PROCESSED_DATA_DIR):
                subpath = clean_path[len(PROCESSED_DATA_DIR):].lstrip('/')
            else:
                subpath = clean_path
...
def _find_processed_dataset_version(original_dataset: Dataset) -> str:
    """
    Trova il path fisico del dataset processed.
    ...
    try:
        ...
        logger.error(f"Errore nel trovare il path del dataset processed: {e}")
        

    # Fallback: usa PROCESSED_DATA_DIR
    logger.warning(f"Path dataset non trovato: {dataset_path}, uso fallback none")
    return None
...
def _load_available_datasets(st_app, dataset_repo: DatasetRepository, uri_prefix: str = None):
    """
    Carica i dataset disponibili del layer PROCESSED (Step 2).
    
    Filtra per:
    - URI prefix (se specificato)
    - Step = 2 (layer PROCESSED)
    """
    try:
        if uri_prefix:
            ...
            # Filtra solo Step 2
            datasets = [d for d in datasets if d.step == 2]
        ...
        logger.info(f"Caricati {len(datasets)} dataset processed (Step 2)")
        return True
    except Exception as e:
        st_app.error("Errore caricando i dataset processed dal DB.")
...
def show_distribution_selection(st_app):
    """
    Interfaccia principale per la selezione di distribuzioni PROCESSED (Layer 2).
    Adattata dalla logica Step 1 per garantire consistenza tra i layer.
    """
    st_app.header("📂 Seleziona una Distribution (Processed Data)")

    # Inizializzazione session state (Sincronizzato con Layer 1)
    st_app.session_state.setdefault("available_datasets", [])
    st_app.session_state.setdefault("selected_path_parts", [])
    st_app.session_state.setdefault("selected_dataset_path", PROCESSED_DATA_DIR)
    st_app.session_state.setdefault("selected_dataset_id", None)

    repos = _initialize_repositories(st_app)

    # 1️⃣ Caricamento dataset (URI match specifico per PROCESSED)
    if not st_app.session_state.available_datasets:
        uri_filter = f"{BASE_PREFIX}{PROCESSED_DATA_DIR}" if PROCESSED_DATA_DIR else None
    ...
    selected_label = st_app.selectbox("Scegli dataset (Processed - Step 2)", dataset_options, key="dataset_select")
    ...
    if selected_label and selected_label in label_to_record:
        ...
        if st_app.session_state.get("selected_dataset_id") != selected_dataset.id:
            ...
            st_app.session_state.selected_dataset_path = _find_processed_dataset_version(selected_dataset)
        ...
        with col_f1:
            dist_search = st_app.text_input("🔍 Cerca distribution (name, description, uri)", key="dist_search_processed").strip().lower()
        with col_f2:
            dist_langs = st_app.multiselect("Lingue", options=lang_options, key="dist_lang_filter_processed")
        with col_f3:
            dist_licenses = st_app.multiselect("Licenze", options=license_options, key="dist_license_filter_processed")
        with col_f4:
            dist_splits = st_app.multiselect("Splits", options=split_options, key="dist_split_filter_processed")
                ...
                try:
                    ...
                    if include_leaf and drec and (dist_langs or dist_licenses or dist_splits):
                        if dist_langs:
                            ...
                            rec_langs = [s.strip().lower() for s in (rec_lang if isinstance(rec_lang, list) else [rec_lang]) if s]
                            if not any(l.lower() in rec_langs for l in dist_langs): include_leaf = False
                except Exception:
                    if dist_search and dist_search not in leaf.lower(): include_leaf = False

                if include_leaf: filtered_leaves.append(leaf)
        ...
        if not filtered_leaves:
            ...
            if len(list_dirs(distribution_internal)) == 0 and len(list_files(distribution_internal)) > 0:
                ...
                if files:
                    ...
                    for f in files: st_app.write(f"- {os.path.basename(f)}")
                
                if st_app.button("✅ Conferma questo dataset come distribuzione", key="confirm_dataset_as_distribution_processed"):
                    _handle_dataset_as_distribution(st_app, distribution_internal, repos['distribution'])
            else:
                st_app.info("Nessuna distribuzione fisica trovata in questo dataset.")
        else:
            ...
            selected_leaf = st_app.selectbox("Distribuzioni rilevate su disco:", options=filtered_leaves, index=def_idx, format_func=lambda x: f"📦 {x}", key="dist_leaf_select_processed")
            ...
            if st_app.button("✅ Conferma questa distribuzione fisica", key="confirm_distribution_processed"):
                dist_uri = f"{BASE_PREFIX}{current_full_path}"
                existing_dist = repos['distribution'].get_by_uri(dist_uri)
                # Validazione specifica per Step 2
                if existing_dist and existing_dist.step != 2:
                    st_app.error(f"🚨 ALERT AMMINISTRATORE: La distribuzione ha step={existing_dist.step}. Deve essere 2 per il layer Processed!")
                else:
        ...
        st_app.divider()
        st_app.subheader("📋 Distribuzioni Logiche (Step 2)")
        
        try:
            dist_uri_filter = f"{BASE_PREFIX}{st_app.session_state.selected_dataset_path}"
            available_dists = repos['distribution'].get_by_uri_prefix_or_step(dist_uri_filter, step=2)
            ...
            for d in unmat:
                ...
                if dist_search:
                    ...
                    if dist_search not in dn and dist_search not in d.uri.lower(): include = False
                if include: unmat_filtered.append(d)

            if unmat_filtered:
                unmat_labels = _build_unmaterialized_distribution_labels(unmat_filtered)
                sel_unmat_label = st_app.selectbox("Scegli distribuzione logica:", [""] + unmat_labels, key="unmat_dist_select_processed")
                ...
                if sel_unmat_label and sel_unmat_label in unmat_map:
                    ...
                    if st_app.button("✅ Usa questa distribuzione logica", key="use_unmaterialized_btn_processed"):
                ...
                st_app.info("Nessuna distribuzione non materializzata con Step 2 trovata.")
        except Exception as e:
            st_app.error(f"Errore caricamento distribuzioni logiche: {e}")

def _create_new_distribution(
    st_app, 
    current_path: str, 
    ...
    try:
        ...
        logger.info(f"Creando nuova distribution con URI: {binded_uri}")
        ...
        if result:
            logger.info(f"✅ Distribution creata - ID: {result.id}")
...
def _confirm_distribution_selection(
    st_app, 
    current_path: str, 
    distribution_repo: DistributionRepository
):
    """Verifica e conferma selezione per il layer Processed (Step 2)."""
    base_path = to_internal_path(_normalize_uri_to_path(st_app.session_state.selected_dataset_uri))
    ...
    logger.info(f"Conferma distribution con URI: {distribution_uri}")

    try:
        existing = distribution_repo.get_by_uri(distribution_uri)
        if existing:
            logger.info(f"Distribution trovata - ID: {existing.id}, Step: {existing.step}")
            ...
            if existing.step == 2:
                ...
                st_app.error(f"Impossibile procedere: Lo step è {existing.step}. Richiesto 2.")
        else:
            logger.info("Distribution non trovata nel DB, creazione...")
